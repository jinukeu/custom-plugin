...
Normally created by `tutor-setup` / `tutor-migrate` with a populated seed block. If `tutor` creates this on the fly (no seed), leave the seed block with the TODO marker — Coverage falls back to file-count heuristic until filled.
